...
std::shared_ptr<nbProject> nbProjectEntitiesLoader::getProjectFromYAML(const YAML::Node &parProjectSupportNodeFile) {
    Version minSupportedVer = parProjectSupportNodeFile[nb::support::YAML_FIELD_NBVERSION].as<std::string>();
    IterVersion curRecipesIterVersion = parProjectSupportNodeFile[nb::support::YAML_FIELD_SUPPORTED_RECIPES_ITERATION_ACTUAL_VERSION].as<std::string>();
    std::string projectName = parProjectSupportNodeFile[nb::support::YAML_FIELD_PROJECT_NAME].as<std::string>();
